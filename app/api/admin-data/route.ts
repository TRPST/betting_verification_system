...
function getRandomStatus() {
  const totalWeight = userData.statuses.reduce(
...
function generateSAIPAddress() {
  const ipRange =
    userData.ipRanges[Math.floor(Math.random() * userData.ipRanges.length)];
  return ipRange.replace(
    "X.X",
...
function generateVerificationData(days: number) {
  ...
  return baseData.map((day, index) => {
    ...
    return {
      ...
      total: Math.floor(
        (day.verified + day.pending + day.flagged + day.fraud) * trendFactor
      ),
    };
  });
}
...
function generateRecentVerifications(count: number) {
  return Array.from({ length: count }, () => {
    ...
    const company =
      Math.random() > 0.3
        ? userData.companies[
            Math.floor(Math.random() * userData.companies.length)
          ]
        : undefined;
    const location =
      userData.locations[Math.floor(Math.random() * userData.locations.length)];
    const type =
      userData.verificationTypes[
        Math.floor(Math.random() * userData.verificationTypes.length)
      ];
    const status = getRandomStatus();
    const idNumber = generateSAIDNumber();
    const deviceType =
      userData.deviceTypes[
        Math.floor(Math.random() * userData.deviceTypes.length)
      ];
    const browser =
      userData.browsers[Math.floor(Math.random() * userData.browsers.length)];
    ...
    const rejectionReason =
      status.includes("⚠️") || status.includes("🚨")
        ? userData.rejectionReasons[
            Math.floor(Math.random() * userData.rejectionReasons.length)
          ]
        : undefined;
...
function generateRejectionStats() {
  return userData.rejectionReasons
    .map((reason) => {
      return {
        reason,
...
function generateStats() {
  ...
  const totalProcessed =
    totalVerified + pendingVerifications + flaggedForReview + fraudAlerts;
  ...
  return {
    ...
    rejectionRate: Math.floor(
      ((flaggedForReview + fraudAlerts) / totalProcessed) * 100
    ),
